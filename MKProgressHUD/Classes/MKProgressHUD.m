...
#error MKProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "MKProgressHUD.h"
#import "MKRadialGradientLayer.h"
#import "MKProgressAnimatedView.h"
#import "MKIndefiniteAnimatedView.h"


NSString * const MKProgressHUMKidReceiveTouchEventNotification = @"MKProgressHUMKidReceiveTouchEventNotification";
NSString * const MKProgressHUMKidTouchDownInsideNotification = @"MKProgressHUMKidTouchDownInsideNotification";
NSString * const MKProgressHUDWillDisappearNotification = @"MKProgressHUDWillDisappearNotification";
NSString * const MKProgressHUMKidDisappearNotification = @"MKProgressHUMKidDisappearNotification";
NSString * const MKProgressHUDWillAppearNotification = @"MKProgressHUDWillAppearNotification";
NSString * const MKProgressHUMKidAppearNotification = @"MKProgressHUMKidAppearNotification";
NSString * const MKProgressHUDStatusUserInfoKey = @"MKProgressHUDStatusUserInfoKey";

static const CGFloat MKProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat MKProgressHUDUndefinedProgress = -1;
static const CGFloat MKProgressHUMKefaultAnimationDuration = 0.15f;
static const CGFloat MKProgressHUDVerticalSpacing = 12.0f;
static const CGFloat MKProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat MKProgressHUDLabelSpacing = 8.0f;
...
@property (nonatomic, strong) MKRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) MKProgressAnimatedView *ringView;
@property (nonatomic, strong) MKProgressAnimatedView *backgroundRingView;
...
+ (MKProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static MKProgressHUD *sharedView;
#if !defined(MK_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(MKProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(MKProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(MKProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:MKProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:MKProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(MKProgressHUDMaskType)maskType {
    MKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(MKProgressHUMKismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(MKProgressHUMKismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = MKProgressHUDMaskTypeNone;
        _defaultStyle = MKProgressHUDStyleLight;
        _defaultAnimationType = MKProgressHUDAnimationTypeNative;
        ...
        _infoImage = [UIImage imageNamed:@"MKHUD.bundle/hud_info"];
        _successImage = [UIImage imageNamed:@"MKHUD.bundle/hud_success"];
        _errorImage = [UIImage imageNamed:@"MKHUD.bundle/hud_error"];
        ...
        _fadeInAnimationDuration = MKProgressHUMKefaultAnimationDuration;
        _fadeOutAnimationDuration = MKProgressHUMKefaultAnimationDuration;
        ...
        // 辅助功能支持
        self.accessibilityIdentifier = @"MKProgressHUD";
...
- (void)updateHUDFrame {
    ...
    // |-间距-内容-间距-|
    hudWidth = MKProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + MKProgressHUDHorizontalSpacing;
    
    // |-间距-内容-(标签间距-标签-)间距-|
    hudHeight = MKProgressHUDVerticalSpacing + labelHeight + contentHeight + MKProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // 如果同时使用内容和标签，则添加间距
        hudHeight += MKProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(MKProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - MKProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != MKProgressHUDUndefinedProgress) {
    ...
    // 标签
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + MKProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-MKProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(MKProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-MKProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(MKProgressHUDParallaxDepthPoints);
...
#else
            // 如果MKProgressHUD用于应用程序扩展，请将其添加到给定视图中
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{MKProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(MK_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(MK_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUMKidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUMKidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak MKProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong MKProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // 重置进度并取消任何正在运行的动画
            strongSelf.progress = MKProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != MKProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        // 发布通知通知用户
        [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // 发布通知通知用户
                [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUMKidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(MKProgressHUMKismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // 发布通知通知用户
            [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // 发布通知通知用户
                    [[NSNotificationCenter defaultCenter] postNotificationName:MKProgressHUMKidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == MKProgressHUDAnimationTypeFlat){
        // 检查SPEnter是否存在并且是不同类的对象
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[MKIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[MKIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        MKIndefiniteAnimatedView *indefiniteAnimatedView = (MKIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (MKProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[MKProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (MKProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[MKProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == MKProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == MKProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == MKProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [MKRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == MKProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == MKProgressHUDMaskTypeCustom){
...
- (void)fadeInEffects {
    if(self.defaultStyle != MKProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == MKProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(MKProgressHUDAnimationType)animationType {
