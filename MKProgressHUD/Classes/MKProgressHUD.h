...
extern NSString * _Nonnull const MKProgressHUMKidReceiveTouchEventNotification;
extern NSString * _Nonnull const MKProgressHUMKidTouchDownInsideNotification;
extern NSString * _Nonnull const MKProgressHUDWillDisappearNotification;
extern NSString * _Nonnull const MKProgressHUMKidDisappearNotification;
extern NSString * _Nonnull const MKProgressHUDWillAppearNotification;
extern NSString * _Nonnull const MKProgressHUMKidAppearNotification;
extern NSString * _Nonnull const MKProgressHUDStatusUserInfoKey;


typedef NS_ENUM(NSInteger, MKProgressHUDStyle) {
    MKProgressHUDStyleLight NS_SWIFT_NAME(light),   // 默认样式，白色抬头显示，黑色文本，抬头显示背景将模糊
    MKProgressHUDStyleDark NS_SWIFT_NAME(dark),     // 黑色抬头显示和白色文本，抬头显示背景将模糊
    MKProgressHUDStyleCustom NS_SWIFT_NAME(custom)  // 自定义 使用前景色和背景色属性
};


typedef NS_ENUM(NSUInteger, MKProgressHUDMaskType) {
    MKProgressHUDMaskTypeNone NS_SWIFT_NAME(none) = 1,      // 默认遮罩类型，在显示HUD时允许用户交互
    MKProgressHUDMaskTypeClear NS_SWIFT_NAME(clear),        // 不允许用户与后台对象交互
    MKProgressHUDMaskTypeBlack NS_SWIFT_NAME(black),        // 不允许用户与背景对象进行交互，并在HUD的后面使UI变暗（如iOS 7和更高版本所示）
    MKProgressHUDMaskTypeGradient NS_SWIFT_NAME(gradient),  // 不允许用户与背景对象交互，并使用a-la UIAlertView背景渐变（如iOS 6所示）来减弱UI
    MKProgressHUDMaskTypeCustom NS_SWIFT_NAME(custom)       // 不允许用户与背景对象交互，并用自定义颜色模糊抬头显示器后面的用户界面
};


typedef NS_ENUM(NSUInteger, MKProgressHUDAnimationType) {
    MKProgressHUDAnimationTypeFlat NS_SWIFT_NAME(flat),     // 默认动画类型，自定义平面动画（不定动画环）
    MKProgressHUDAnimationTypeNative NS_SWIFT_NAME(native)  // iOS native UIActivityIndicatorView
};

typedef void (^MKProgressHUDShowCompletion)(void);
typedef void (^MKProgressHUMKismissCompletion)(void);
...
@property (assign, nonatomic) MKProgressHUDStyle defaultStyle UI_APPEARANCE_SELECTOR;                   // 默认MKProgressHUDStyleLight
@property (assign, nonatomic) MKProgressHUDMaskType defaultMaskType UI_APPEARANCE_SELECTOR;             // 默认MKProgressHUDMaskTypeNone
@property (assign, nonatomic) MKProgressHUDAnimationType defaultAnimationType UI_APPEARANCE_SELECTOR;   //默认LMProgressHUDAnimationTypeNative
...
+ (void)setDefaultStyle:(MKProgressHUDStyle)style;                      // 默认LMProgressHUDStyleLight
+ (void)setDefaultMaskType:(MKProgressHUDMaskType)maskType;             // 默认LMProgressHUDMaskTypeNone
+ (void)setDefaultAnimationType:(MKProgressHUDAnimationType)type;       // 默认LMProgressHUDAnimationTypeNative
...
// 隐藏完成回调
+ (void)dismissWithCompletion:(nullable MKProgressHUMKismissCompletion)completion;

// 延迟隐藏
+ (void)dismissWithDelay:(NSTimeInterval)delay;

// 延迟隐藏完成回调
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(nullable MKProgressHUMKismissCompletion)completion;
